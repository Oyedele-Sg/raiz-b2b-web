...
Props) => {
  const { amount, swapToCurrency, swapFromCurrency } = useCryptoSwapStore();
  const user = {
    entity_id: "",
    account_name: "",
    username: "",
    ...
    switch (status) {
      case "loading":
        return (
          <LoadingStatus
            user={user}
            loadingText={`Swapping ${amount} ${swapFromCurrency} to ${swapToCurrency}`}
            type="p2p"
          />
        );
      case "success":
...
export default CryptoSwapStatusModal;
