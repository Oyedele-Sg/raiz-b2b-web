...
const settingsSchema = z.object({
  discountOption: z.enum(["none", "lineItem", "transaction"]),
  taxOption: z.enum(["inclusive", "exclusive", "either"]),
  terms: z
    .string()
              ...
              <div>
                ...
                <div className="space-y-3 text-sm">
                  {[
                    ...
                    { value: "lineItem", label: "At Line Item level" },
                    { value: "transaction", label: "At Transactional Level" },
                  ].map((opt) => (
                    <label
                      key={opt.value}
                    ...
                    >
                      <Radio
                        // name="discountOption"
                        checked={formik.values.discountOption === opt.value}
                        onChange={() =>
                          formik.setFieldValue("discountOption", opt.value)
                        }
                      />
                      <span>{opt.label}</span>
                    </label>
              ...
              <div>
                ...
                <div className="space-y-3 text-sm">
                  {[
                    ...
                    { value: "either", label: "Tax Inclusive or Exclusive" },
                  ].map((opt) => (
                    <label
                      key={opt.value}
                      className="flex items-center gap-3 cursor-pointer"
                    >
                      <Radio
                        // name="taxOption"
                        checked={formik.values.taxOption === opt.value}
                        onChange={() =>
                          formik.setFieldValue("taxOption", opt.value)
                        }
                      />
                      <span>{opt.label}</span>
                    </label>
